...
const TrackCard = ({ trackURI, index, total, setTrackURIs, playerRef }) => {
  ...
  const handleDragEnd = () => {
    if (x.get() > 100) {
      setTrackURIs((prev) => prev.filter((uri) => uri !== trackURI));
      handleSwipeRight();
    }
    if (x.get() < -100) {
      setTrackURIs((prev) => prev.filter((uri) => uri !== trackURI));
  ...
  console.log("TrackCard rendered: " + trackInfo.name);
